...
public final class LiveLifecycle<T> implements ObservableTransformer<T, T>, LifecycleObserver {

    private static final String TAG = "LiveLifecycle";

    public static <T> ObservableTransformer<T, T> bindToLifecycle(LifecycleOwner owner) {
        return new LiveLifecycle<>(owner);
    }
    ...
    private LiveLifecycle(LifecycleOwner owner) {
    ...
    void onStateChange() {
        if (this.mLifecycleOwner.getLifecycle().getCurrentState() == Lifecycle.State.DESTROYED) {
            if (mDisposable != null && !mDisposable.isDisposed()) {
                Log.i(TAG, "disposeCurrent upstream");
                mDisposable.dispose();
            }
            mLifecycleOwner.getLifecycle().removeObserver(this);
        } else {
            this.activeStateChanged(LiveLifecycle.isActiveState(mLifecycleOwner.getLifecycle().getCurrentState()));
        }
    }
